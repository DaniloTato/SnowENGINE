...
RuntimeValue addScriptCommand(const CommandContext &ctx) {
  ...
  // SnowlangHelper::RuntimeValueTo<std::string>(ctx.cmd.span)(ctx.args[1]); const auto &scriptName
  // = SnowlangHelper::RuntimeValueTo<std::string>(ctx.cmd.span)(ctx.args[2]);
